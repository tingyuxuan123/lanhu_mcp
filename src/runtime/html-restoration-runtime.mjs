...
const outputPrefix = options.outputPrefix || process.env.RESTORATION_OUTPUT_PREFIX || (pageUrl || directJsonUrl ? 'lanhu-restoration' : 'sample-restoration');
const localAssetDirName = `${outputPrefix}-assets`;
const localAssetDir = path.join(outputDir, localAssetDirName);
const statusTimeLabel = options.statusTime || process.env.SAMPLE_STATUS_TIME || '1:21 AM';
const statusAppLabel = options.statusApp || process.env.SAMPLE_STATUS_APP || 'WeChat';
...
function preciseFlowLayerStyles() {
  return [
    'position:relative',
    'box-sizing:border-box',
  ].join(';');
...
function preciseGridContentLayerStyles() {
  return [
    'grid-area:1 / 1',
    'z-index:1',
    'position:relative',
...
function shouldUsePreciseLineLayout(line, items) {
  if (!line || items.length < 2) {
    return false;
  }

  const topOffsets = items.map(item => item.bounds.y - line.bounds.y);
  const maxTopOffset = Math.max(...topOffsets);
  const minTopOffset = Math.min(...topOffsets);
  const heights = items.map(item => item.bounds.height);
  const maxHeight = Math.max(...heights);
  const minHeight = Math.min(...heights);

  return (maxTopOffset - minTopOffset) > 2.5 || (maxHeight - minHeight) > 4;
}

function shouldUsePreciseFlexFlowLayout(node, items, mode = 'absolute') {
  const layout = node.layoutHint;
  if (mode !== 'flow' || !layout || layout.lines?.length || items.length < 2 || items.length > 8) {
    return false;
  }

  const allInsideBounds = items.every(item => isBoundsInside(node.bounds, item.bounds, 2));
  if (!allInsideBounds) {
    return false;
  }

  if (layout.mode === 'flex-row') {
    const topOffsets = items.map(item => item.bounds.y - node.bounds.y);
    const maxTopOffset = Math.max(...topOffsets);
    const minTopOffset = Math.min(...topOffsets);
    const heights = items.map(item => item.bounds.height);
    const maxHeight = Math.max(...heights);
  ...
  if (layout.mode === 'flex-column') {
    const leftOffsets = items.map(item => item.bounds.x - node.bounds.x);
    const maxLeftOffset = Math.max(...leftOffsets);
    const minLeftOffset = Math.min(...leftOffsets);
    const widths = items.map(item => item.bounds.width);
    const maxWidth = Math.max(...widths);
    const minWidth = Math.min(...widths);
    return (maxLeftOffset - minLeftOffset) > 2.5 || (maxWidth - minWidth) > 8;
  }

  return false;
}

function renderPreciselyPositionedChildren(parentBounds, children, insideMask = false) {
  return children.map(child => renderNode(child, parentBounds.x, parentBounds.y, 'absolute', insideMask)).join('\n');
}

function renderLayoutLines(node, insideMask = false) {
  const layout = node.layoutHint;
  ...
  return lines.map((line, index) => {
    ...
    const preciseLineLayout = shouldUsePreciseLineLayout(line, items);
    if (preciseLineLayout) {
      styles.push('position:relative');
      styles.push(`width:${line.bounds.width}px`);
      styles.push(`height:${line.bounds.height}px`);
    ...
    const lineHtml = preciseLineLayout
      ? renderPreciselyPositionedChildren(line.bounds, items, insideMask)
...
function renderFlowNode(node) {
  ...
  if (node.layoutHint && node.layoutHint.mode !== 'absolute') {
    return renderFlexNode(node, 0, 0, 'flow', false);
  }
...
function renderFlexNode(node, offsetX = 0, offsetY = 0, mode = 'absolute', insideMask = false) {
...
function renderNode(node, offsetX = 0, offsetY = 0, mode = 'absolute', insideMask = false) {
  ...
  if (node.layoutHint && node.layoutHint.mode !== 'absolute') {
    return renderFlexNode(node, offsetX, offsetY, mode, insideMask);
  }
